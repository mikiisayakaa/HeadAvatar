...
from mscene import Scene, MeshModelTexEdit
from argparse import ArgumentParser
from arguments import MeshModelTexEditParams
...
def test(mp, device):
    ...
    mesh_model = MeshModelTexEdit(mp, scene.flame_info, device)
    
    os.makedirs("editing_results", exist_ok=True)
    ...
    for timestep in mp.timesteps:
        ...
        for cam_batch in val_loader:
            ...
            iu.export_img(os.path.join("editing_results", f"{str(timestep).zfill(5)}_{str(cam_batch.camera_id[0]).zfill(2)}.png"), image)      

            
if __name__ == "__main__":
    parser = ArgumentParser()
    parser.add_argument("--gpuID", type=int, default=0)
    mp = MeshModelTexEditParams(parser)
