...
__constant__ float c_envmap[3 * 16 * 32];
...
template<int C>
__global__ void __launch_bounds__(256) BruteForceSpecularShaderKernel_Forward2_Batch(
    int B, int H, int W, int env_H, int env_W,
    const float* __restrict__ normal,         // B*H*W x 3
    const float* __restrict__ view_dir,       // B*H*W x 3
    const float* __restrict__ albedo,         // B*H*W x C
    const float* __restrict__ specular_albedo,// B*H*W x C
    const float* __restrict__ roughness,      // B*H*W x 1
    const float* __restrict__ light2obj_rotmat,// 3x3  (or B x 3x3 if per-batch)
    const bool*  __restrict__ mask,          // B*H*W bool mask
    float* __restrict__ shading,             // B*H*W x C
    float* __restrict__ diffuse_shading,
    bool enable_specular
){
    // max env_map size: 32 * 64

    // 计算全局像素索引
    int pix_idx = blockIdx.x * blockDim.x + threadIdx.x;
    int total_pix = B * H * W;
    bool valid = pix_idx < total_pix && mask[pix_idx]; // mask判断
    int b = pix_idx / (H*W);           // batch索引
    int hw = pix_idx % (H*W);
    int y = hw / W;
    int x = hw % W;
    int N_texel = env_H * env_W;
    ...
    #pragma unroll
    for(int i = 0; i < 9; i++)
        l_mat[i] = light2obj_rotmat[b * 9 + i];

    ////////////// load normal
    l_normal.x = normal[pix_idx*3 + 0];
    l_normal.y = normal[pix_idx*3 + 1];
    l_normal.z = normal[pix_idx*3 + 2];


    ////////////// load albedo
    #pragma unroll
    for(int i = 0; i < C; i++){
        l_albedo[i] = albedo[pix_idx*C + i];
    }
    ...
    if (enable_specular){
        #pragma unroll
        for(int i = 0; i < C; i++){
            l_specular_albedo[i] = specular_albedo[pix_idx*C + i];
        }
        l_roughness = roughness[pix_idx];
        alpha0 = l_roughness * l_roughness;
        alpha0 = alpha0 * alpha0;
        alpha1 = (l_roughness + 1.f);
        alpha1 = (alpha1*alpha1)/8.f;

        l_viewdir.x = view_dir[pix_idx*3 + 0];
        l_viewdir.y = view_dir[pix_idx*3 + 1];
        l_viewdir.z = view_dir[pix_idx*3 + 2];
    }
    ...
    for (int i = 0; i < C; i++){
        l_shading[i] = 0;
        l_diffuse_shading[i] = 0;
    }

    for(int j = 0; j < N_texel; j++){
        int ix = j % env_W;
        int iy = j / env_W;
#ifdef LEFT_TOP_AS_ORIGIN
        iy = env_H - 1 - iy;
#endif
        float3 l_dir = getLightDir(iy, ix, env_W, env_H);
        float delta_omega = getDeltaOmega(iy, env_W, env_H);
        ...
        #pragma unroll
        for(int i_channel = 0; i_channel < C; i_channel++){
            l_envmap[i_channel] = c_envmap[N_texel * i_channel + j];
        }
    ...
    #pragma unroll
    for(int i = 0; i < C; i++){
        shading[pix_idx*C + i] = l_shading[i];
        diffuse_shading[pix_idx*C + i] = l_diffuse_shading[i];
    }
}
...
template<int C>
__global__ void __launch_bounds__(256) BruteForceSpecularShaderKernel_Backward2_Batch(
    int B, int H, int W, int env_H, int env_W,
    const float* __restrict__ grad_shading,         // B*H*W x C
    const float* __restrict__ grad_diffuse_shading, // B*H*W x C
    const float* __restrict__ normal,               // B*H*W x 3
    const float* __restrict__ view_dir,             // B*H*W x 3
    const float* __restrict__ albedo,               // B*H*W x C
    const float* __restrict__ specular_albedo,      // B*H*W x C
    const float* __restrict__ roughness,            // B*H*W x 1
    const float* __restrict__ light2obj_rotmat,     // 3x3
    const bool* __restrict__ mask,                  // B*H*W mask
    float* __restrict__ grad_normal,                // B*H*W x 3
    float* __restrict__ grad_view_dir,             // B*H*W x 3
    float* __restrict__ grad_albedo,               // B*H*W x C
    float* __restrict__ grad_specular_albedo,      // B*H*W x C
    float* __restrict__ grad_roughness,            // B*H*W x 1
    bool enable_specular
){
    int N_texel = env_H * env_W;
    ...
    int pix_idx = blockIdx.x * blockDim.x + threadIdx.x;
    int total_pix = B * H * W;
    bool valid = pix_idx < total_pix && mask[pix_idx];
    int b = pix_idx / (H*W);
    int hw = pix_idx % (H*W);
    int y = hw / W;
    int x = hw % W;
    ...
    float3 l_viewdir;
    float3 l_normal;
    float l_albedo[C], l_specular_albedo[C];
    float l_roughness, alpha0, alpha1;
    float l_grad_shading[C], l_grad_diffuse_shading[C];
    float l_grad_normal[3], l_grad_viewdir[3], l_grad_albedo[C], l_grad_specular_albedo[C];
    float grad_alpha0, grad_alpha1;
    // ================================================================

    for(int i=0;i<9;i++) l_mat[i]=light2obj_rotmat[b * 9 + i];
    ...
    if(enable_specular){
        ...
        for(int i=0;i<C;i++)
            l_specular_albedo[i] = specular_albedo[pix_idx*C + i];
        l_roughness = roughness[pix_idx];
        alpha0 = l_roughness*l_roughness; alpha0 = alpha0*alpha0;
        alpha1 = (l_roughness + 1.f); alpha1 = alpha1*alpha1/8.f;
        l_viewdir.x = view_dir[pix_idx*3 + 0];
        l_viewdir.y = view_dir[pix_idx*3 + 1];
        l_viewdir.z = view_dir[pix_idx*3 + 2];
    }

    ////////////// load grad_shading & grad_diffuse_shading
    #pragma unroll
    for(int i=0;i<C;i++){
        l_grad_shading[i] = grad_shading[pix_idx*C + i];
        l_grad_diffuse_shading[i] = grad_diffuse_shading[pix_idx*C + i];
    }
    ...
    for(int i=0;i<C;i++){
        l_grad_albedo[i]=0;
        if(enable_specular) l_grad_specular_albedo[i]=0;
    }
    if(enable_specular){
        l_grad_viewdir[0]=0; l_grad_viewdir[1]=0; l_grad_viewdir[2]=0;
        grad_alpha0=0; grad_alpha1=0;
    ...
    for(int j = 0; j < N_texel; j++){
        ...
        #pragma unroll
        for(int i_channel=0;i_channel<C;i_channel++)
            l_envmap[i_channel] = c_envmap[i_channel * N_texel + j];
    ...
    ////////////// store gradients
    for(int i=0;i<3;i++) grad_normal[pix_idx*3 + i] = l_grad_normal[i];
    for(int i=0;i<C;i++) grad_albedo[pix_idx*C + i] = l_grad_albedo[i];
    if(enable_specular){
        for(int i=0;i<3;i++) grad_view_dir[pix_idx*3 + i] = l_grad_viewdir[i];
        for(int i=0;i<C;i++) grad_specular_albedo[pix_idx*C + i] = l_grad_specular_albedo[i];
        float l_grad_r = grad_alpha0*4.f*l_roughness*l_roughness*l_roughness + grad_alpha1*(1.f+l_roughness)/4.f;
        grad_roughness[pix_idx] = l_grad_r;
    }
}
...
template<int C>
__global__ void __launch_bounds__(256) BruteForceSpecularShaderKernel_BackwardEnvmap2_Batch(
    int B, int H, int W, int env_H, int env_W,
    const float* __restrict__ normal,            // B*H*W x 3
    const float* __restrict__ view_dir,          // B*H*W x 3
    const float* __restrict__ albedo,            // B*H*W x C
    ...
    const float* __restrict__ grad_shading,      // B*H*W x C
    const float* __restrict__ grad_diffuse_shading, // B*H*W x C
    const float* __restrict__ light2obj_rotmat,  // B x 3 x 3
    const bool* __restrict__ mask,
    float* __restrict__ grad_envmap,             // C x env_H x env_W
    bool enable_specular
){
    int N_texel = env_H * env_W;
    int pix_idx = blockIdx.x * blockDim.x + threadIdx.x;
    ...
    float3 l_normal;
    float3 l_viewdir;
    float l_diffuse_grad[3];
    float l_specular_grad[3];
    float l_roughness;
    float l_mat[9];
    float l_grad_envmap[C];

    ////////////// load light2obj matrix
    #pragma unroll
    for(int i=0;i<9;i++)
        l_mat[i] = light2obj_rotmat[b*9 + i];
    ...
    l_normal.x = normal[pix_idx*3];
    l_normal.y = normal[pix_idx*3 + 1];
    l_normal.z = normal[pix_idx*3 + 2];

    if(enable_specular){
        l_viewdir.x = view_dir[pix_idx*3];
        l_viewdir.y = view_dir[pix_idx*3 + 1];
        l_viewdir.z = view_dir[pix_idx*3 + 2];
    }
    ...
    for(int i=0;i<C;i++){
        int idx = pix_idx*C + i;
        float l_grad = grad_shading[idx];
        float l_diffuse_shading_grad = grad_diffuse_shading[idx];
        float l_a = albedo[idx];
        l_diffuse_grad[i] = l_grad * l_a + l_diffuse_shading_grad;

        if(enable_specular){
            float l_spec_a = specular_albedo[idx];
            l_specular_grad[i] = l_grad * l_spec_a;
        }
    }

    ////////////// accumulate grad for envmap
    ...
    for(int j=0; j<N_texel; j++){
        ...
        if(cos_shade > 0){
            #pragma unroll
            for(int i_channel=0;i_channel<C;i_channel++)
                l_grad_envmap[i_channel] += l_diffuse_grad[i_channel] * cos_shade/M_PIf * delta_omega;
        }

        if(enable_specular){
            float r = l_roughness;
            float alpha0 = r*r; alpha0 = alpha0*alpha0;
            float alpha1 = (r+1.f); alpha1 = alpha1*alpha1/8.f;
            float3 h = normalize(add(rot_l_dir, l_viewdir));
            float cosh = dot(rot_l_dir, h);
            float F = FresnelApproximate(F0, cosh);
            float G = G1_robust(rot_l_dir, l_viewdir, l_normal, alpha1);
            float D = NDF1(l_normal, h, alpha0);
            ...
            if(tmp>0){
                #pragma unroll
                for(int i_channel=0;i_channel<C;i_channel++)
                    l_grad_envmap[i_channel] += l_specular_grad[i_channel] * tmp * delta_omega;
            }
        }

        #pragma unroll
        for(int i_channel=0;i_channel<C;i_channel++)
            atomicAdd(grad_envmap + i_channel*N_texel + j, l_grad_envmap[i_channel]);
    }
}
...
std::vector<torch::Tensor> bruteforce_specular_shader_forward2_batch(
    const torch::Tensor & normal,         // B x H x W x 3
    const torch::Tensor & view_dir,       // B x H x W x 3
    const torch::Tensor & albedo,         // B x H x W x C
    const torch::Tensor & specular_albedo,// B x H x W x C
    const torch::Tensor & roughness,      // B x H x W x 1
    const torch::Tensor & envmap,         // C x H x W
    const torch::Tensor & light2obj_rotmat,// 3 x 3
    const torch::Tensor & mask,           // B x H x W, bool
    bool enable_specular
){
    CHECK_CUDA(normal); CHECK_CONTIGUOUS(normal); CHECK_IS_FLOATING(normal);
    CHECK_CUDA(view_dir); CHECK_CONTIGUOUS(view_dir); CHECK_IS_FLOATING(view_dir);
    ...
    CHECK_CUDA(envmap); CHECK_CONTIGUOUS(envmap); CHECK_IS_FLOATING(envmap);
    CHECK_CUDA(light2obj_rotmat); CHECK_CONTIGUOUS(light2obj_rotmat); CHECK_IS_FLOATING(light2obj_rotmat);
    CHECK_CUDA(mask); CHECK_CONTIGUOUS(mask); // mask bool

    int B = normal.size(0);
    int H = normal.size(1);
    int W = normal.size(2);
    int env_H = envmap.size(1);
    int env_W = envmap.size(2);
    int C = albedo.size(3);
    ...
    torch::Tensor shading = torch::zeros({B, H, W, C}, opt);
    torch::Tensor diffuse_shading = torch::zeros({B, H, W, C}, opt);

    int64_t total_pix = B * H * W;
    const uint32_t THREADS = 256;
    const uint32_t BLOCKS = (uint32_t)div_round_up(total_pix, (int64_t)THREADS);

    cudaMemcpyToSymbol(c_envmap, envmap.data_ptr<float>(), sizeof(float) * 3 * 16 * 32);

    if(BLOCKS){
        if(C == 3){
            BruteForceSpecularShaderKernel_Forward2_Batch<3><<<BLOCKS, THREADS>>>(
                B, H, W, env_H, env_W,
                normal.data_ptr<float>(),
                view_dir.data_ptr<float>(),
                albedo.data_ptr<float>(),
...
std::vector<torch::Tensor> bruteforce_specular_shader_backward2_batch(
    const torch::Tensor & grad_shading,        // B x H x W x C
    const torch::Tensor & grad_diffuse_shading,// B x H x W x C
    const torch::Tensor & normal,              // B x H x W x 3
    const torch::Tensor & view_dir,            // B x H x W x 3
    const torch::Tensor & albedo,              // B x H x W x C
    const torch::Tensor & specular_albedo,     // B x H x W x C
    ...
    int H = normal.size(1);
    int W = normal.size(2);
    int env_H = (int)envmap.size(1);
    int env_W = (int)envmap.size(2);
    int C = albedo.size(3);
    ...
    torch::Tensor grad_normal = torch::zeros({B, H, W, 3}, opt);
    torch::Tensor grad_albedo = torch::zeros({B, H, W, C}, opt);
    torch::Tensor grad_envmap = torch::zeros({C, env_H, env_W}, opt);
    torch::Tensor grad_view_dir = enable_specular ? torch::zeros({B,H,W,3}, opt) : torch::zeros({B,H,W,3}, opt);
    torch::Tensor grad_specular_albedo = enable_specular ? torch::zeros({B,H,W,C}, opt) : torch::zeros({B,H,W,C}, opt);
    torch::Tensor grad_roughness = enable_specular ? torch::zeros({B,H,W,1}, opt) : torch::zeros({B,H,W,1}, opt);
    ...
    if(BLOCKS){
        if(C == 3){
            BruteForceSpecularShaderKernel_Backward2_Batch<3><<<BLOCKS, THREADS>>>(
                B, H, W, env_H, env_W,
                grad_shading.data_ptr<float>(),
                grad_diffuse_shading.data_ptr<float>(),
                normal.data_ptr<float>(),
            ...
            BruteForceSpecularShaderKernel_BackwardEnvmap2_Batch<3><<<BLOCKS, THREADS>>>(
                B, H, W, env_H, env_W,
                normal.data_ptr<float>(),
                view_dir.data_ptr<float>(),
                albedo.data_ptr<float>(),
