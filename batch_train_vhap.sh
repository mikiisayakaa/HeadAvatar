...
names=("253" "264" "302" "304" "306")

for name in "${names[@]}"; do
    # echo "Training single frame for subject $name"    
    DATA_DIR=./data/${name}
    BASE_DIR=./output/paper_${name}_base
    ...
    MULTI_DIR=./output/paper_${name}_seq_vhap
    echo "NAME = '$name'"
    echo "DATA_DIR = '$DATA_DIR'"
    $PYTHON train_seq.py -s ${DATA_DIR} -m ${MULTI_DIR} -l ${BASE_DIR} --eval --gpuID ${DEVICE} --vhap
done
