...
import sys
from mscene import Scene, MeshModelBatchTex
...
from arguments import MeshModelSequenceTexParams, TrainConfig
...
def training_mesh(mp, device):
    ...
    mesh_model = MeshModelBatchTex(args=mp, flame_info=scene.flame_info, device=device)
    ...
    for epoch in range(first_iter, mp.epoch + 1):
        ...
        for cam_batch in train_loader:
            ...
            with timer.record("Losses: "):
                ...
                L_lpips = VGGLoss(image, gt_image, epoch)
                
                shader_reg = lu.shader_regularization(mesh_model.shader.adaptive, mesh_model.shader.neural_shader, renderer.texc[0], device, epoch) if mesh_model.neural else 0
                ...
                albedo_tv = lu.tv_loss(renderer.colors) if do_texture and mp.lambda_albedo_tv > 0 else 0
                ...
                loss = (mp.lambda_l1 * Ll1 + 
                        ...
                        mp.lambda_albedo_tv * albedo_tv +
                        mp.lambda_roughness_tv * roughness_tv +
                        mp.lambda_roughness * roughness_reg) * lr_boost
...
if __name__ == "__main__":
    ...
    mp = MeshModelSequenceTexParams(parser)
