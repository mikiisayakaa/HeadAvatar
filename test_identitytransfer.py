...
from mscene import Scene, MeshModelIdentityTransfer
from argparse import ArgumentParser
from arguments import MeshModelIdentityTransferParams
...
def test(mp, device):
    ...
    mesh_model = MeshModelIdentityTransfer(mp, scene.flame_info, device)
    
    os.makedirs("id_transfer_results", exist_ok=True)
    ...
    for timestep in mp.timesteps:
        ...
        for cam_batch in val_loader:
            ...
            iu.export_img(os.path.join("id_transfer_results", f"{str(timestep).zfill(5)}_{str(cam_batch.camera_id[0]).zfill(2)}.png"), image)     
...
if __name__ == "__main__":
    ...
    mp = MeshModelIdentityTransferParams(parser)
