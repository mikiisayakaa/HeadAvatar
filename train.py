...
import sys
from mscene import Scene, MeshModel
from utils.general_utils import safe_state
from utils.image_utils import psnr, error_map, visualize_grid, get_semantic_image
...
from arguments import MeshModelParams, TrainConfig
try:
...
def set_bits(mp, iteration, do_vertex, do_texture, do_semantic):
    ...
    if iteration >= mp.texture_epoch[0] and iteration < mp.texture_epoch[1]:
        do_texture = True
    if iteration >= mp.semantic_epoch[0] and iteration < mp.semantic_epoch[1]:
        do_semantic = True
...
def training_mesh(mp, device):
    ...
    mesh_model = MeshModel(mp, scene.flame_info, device=device)
    ...
    for epoch in range(first_iter, mp.epoch + 1):
        ...
        for cam_batch in train_loader:
            ...
            with timer.record("Calculate losses: "):
                ...
                Lsemantic = lu.l1_loss(renderer.learnable_semantic, cam_batch.semantic) if do_semantic else lu.l1_loss(renderer.learnable_semantic, cam_batch.semantic_train)
                
                semantic_reg = (lu.sum_loss(mesh_model.get_semantic_tex()) + lu.range_loss(mesh_model.get_semantic_tex())) if mp.lambda_semantic_reg > 0 else 0
                semantic_tv = lu.tv_loss(mesh_model.get_semantic_tex()) if mp.lambda_semantic_tv > 0 else 0
                ...
                loss = (mp.lambda_l1 * Ll1 + 
                        ...
                        mp.lambda_semantic_reg * semantic_reg +
                        mp.lambda_semantic_tv * semantic_tv +
                        mp.lambda_lpips * L_lpips +
                        mp.lambda_shader_reg * shader_reg) * lr_boost
        ...
        with timer.record("Logging: "):
            with torch.no_grad():
                loss_dict = {
                    ...
                    "semantic_reg": semantic_reg,
                    "semantic_tv": semantic_tv,
...
def training_report(tb_writer, val_loader, epoch, losses, elapsed, mesh_model, device, mp):
    ...
    if epoch % mp.check == 0:
        ...
        for idx, camera in enumerate(val_loader):
            ...
            # shading_img = renderer.shading * renderer.mask
            if epoch < mp.semantic_epoch[0]:
                semantic_gt = get_semantic_image(camera.semantic_train)
            else:
        ...
        albedo = mesh_model.get_albedo()
        roughness = mesh_model.get_roughness().repeat(1, 1, 1, 3)
        envmap = mesh_model.get_light()[None].permute(0, 3, 1, 2).repeat(1, 3, 1, 1)
        envmap = F.interpolate(envmap, scale_factor=albedo.shape[1] / envmap.shape[3] * 0.9, mode='bilinear').permute(0, 2, 3, 1)
        envmap_canvas = torch.zeros_like(albedo)
        hstart = (envmap_canvas.shape[1] - envmap.shape[1]) // 2
        wstart = (envmap_canvas.shape[2] - envmap.shape[2]) // 2
        envmap_canvas[:, hstart:hstart+envmap.shape[1], wstart:wstart+envmap.shape[2], :] = envmap

        semantic_tex = get_semantic_image(mesh_model.get_semantic_tex())
        ...
        visualize_grid(images_texture, nrow=2, save_path=prefix + "_texture.png")
